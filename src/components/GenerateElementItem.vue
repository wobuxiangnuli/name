  ...
  <div :class="{'print-read-label': printRead}" style="width: 100%;">
    ...
    <template v-if="widget.type == 'steps'">
      <el-steps :active="dataModel" 
        ...
        v-if="widget.options.remote ? (remoteOptions && remoteOptions.length > 0) : true"
        v-bind="widget.options.customProps"
      >
        <el-step 
          :title="widget.options.remote ? item.value : item.title" 
          :description="widget.options.remote ? item.label : item.description" 
          v-for="(item, index) in (widget.options.remote ? remoteOptions : widget.options.steps)" :key="index"></el-step>
      </el-steps>
    </template>
